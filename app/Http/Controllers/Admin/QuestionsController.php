...
class QuestionsController extends Controller
{
    ...
    public function store(StoreQuestion $request)
    {
        ...
        $sanitized['type'] = $request->getTypeId();
        $splitted = explode(",", $sanitized['options']);
        $sanitized['options'] = json_encode($splitted);
        $splittedRules = explode(",", $sanitized['rules']);
        $sanitized['rules'] = json_encode($splittedRules);
        //return $splitted;
        //$sanitized['settings'] = json_encode($splitted);
        //$sanitized['rules'] = '['.$sanitized['rules'].']';
        ...
        if ($request->ajax()) {
            return ['redirect' => url('admin/surveys/'.$request['survey_id'].'/show'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }
    ...
    public function update(UpdateQuestion $request, Question $question)
    {
        ...
        if ($request->ajax()) {
            return [
                'redirect' => url('admin/surveys/'.$request['survey_id'].'/show'),
